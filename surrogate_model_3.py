...
from scipy.signal import windows # Make sure to add this import at the top of your script
...
def generate_fd_waveform(params, f_lower, delta_t, nfft):
    ...
    # --- THIS IS THE FIX: Apply a window function before padding ---
    # A Tukey window is flat in the middle and tapers at the ends.
    # We apply it only to the actual signal portion.
    # alpha=0.1 means 5% of each end will be tapered.
    tukey_window = windows.tukey(len(h_td_raw), alpha=0.1)
    h_td_windowed = h_td_raw * tukey_window
    
    # Pad the windowed signal to the full nfft length
