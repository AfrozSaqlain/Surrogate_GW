...
def get_amp_phase(freqs, h_fd):
    """Extracts amplitude and unwrapped phase, and removes a linear fit from the phase."""
    amp = np.abs(h_fd)
    phase = np.unwrap(np.angle(h_fd))

    # Subtract a linear fit from the phase to remove arbitrary time/phase shifts [cite: 267]
    poly_fit = np.polyfit(freqs, phase, 1)
    linear_phase = np.polyval(poly_fit, freqs)
    phase_centered = phase - linear_phase

    return amp, phase_centered
...
chi_vals = np.linspace(-0.8, 0.6, 50)
